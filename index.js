...
var Calendar = React.createClass({
	...
	addEntry: function addEntry(day) {
		if (this.state.currDay) {
			...
			return this.setState({ warning: "Seleccione un día si quiere hacer una reserva!" });
		}
	},
	saveEntry: function saveEntry(year, month, day) {
		var entryName = $("#entry_name").val();
		if ($.trim(entryName).length > 0) {
            var startx=document.URL.length;
            var userx=document.URL.substring(45,startx);
            entryName=userx+': '+entryName;
			...
			} else if ($("#enter_hour1").val() && $("#enter_hour1").val() >= 0 && $("#enter_hour1").val() <= 24 && $("#enter_hour2").val() && $("#enter_hour2").val() >= 0 && $("#enter_hour2").val() <= 24 && $("#enter_hour2").val()<=$("#enter_hour1").val()) {
				var entryDuration = addZero($("#enter_hour2").val())+' - '+addZero($("#enter_hour1").val());
			} else {
			...
			if ($("#entry_location").val()) {
				var entryLocation = $("#entry_location").val();
 				if (entryLocation != "Piscina" && entryLocation != "Zona de bar" && entryLocation != "Zona social" && entryLocation != "Zona infantil"){
					window.alert("Solo puede escribir Piscina, Zona de bar, Zona social o Zona infantil");
					var entryLocation = "";
return 0;}
			} else {
				var entryLocation = "";
            ...
            var start=document.URL.length;
            var user=document.URL.substring(45,start);

			$.post('bd.php', { entryName,entryDate,entryTime,entryLocation,user,entryDuration }, function(result) {
	   			alert(result);
			});
	...
	DataBase: function DataBase() {
		...
		$.post('bdload.php', {  }, function(result) {

			var entryName = 'AiuDA';

			
			//alert(result);
			//$("#entry_nameU").val(result);
			//window.alert('pre');
			//window.alert(entryName);
			var temp=new String(result);
			entryName=temp.split(",");
			//window.alert('post');
			//window.alert(entryName[6]);

			var length=entryName.length;
			var i=1;
			//window.alert(length);
			...
			if (length>2) {
				while (i<length-1){
					var entryTime = new Date();
					var entryDate={ year: parseInt(entryName[i+4]), month: parseInt(entryName[i+3]), day: parseInt(entryName[i+2]) };
					$(".duration").css('background', 'none');
					var entryDuration = entryName[i+5];
					//var entryDuration = addZero($("#enter_hour2").val());
					var entryLocation=entryName[i+1];
					//var entryLocation = $("#entry_location").val();
					if ($("#entry_note").val()) {
						var entryNote = $("#entry_note").val();
					} else {
					...
					var entryImg = x.state.file;
					var entryColor = x.state.dColor;
					
					var entry = { entryName: entryName[i], entryDate: entryDate, entryTime: entryTime, entryDuration: entryDuration, entryLocation: entryLocation, entryNote: entryNote, entryColor: entryColor, entryImg: entryImg };
					x.state.entries.splice(0, 0, entry);
					// clean and close entry page
					$(".float").removeClass('rotate');
					$("#add_entry").addClass('animated slideOutDown');
					window.setTimeout(function () {
					...
					/*$.post('bd.php', { entryName,entryDate,entryTime,entryLocation }, function(result) {
			   			alert(result);
					});*/

					x.setState({ entries: x.state.entries }), x.setState(resColor);
					//window.alert('fin');
					i=i+6;
				}
			}
	...
	deleteEntry: function deleteEntry(e) {
		var jsday=this.state.currDay;
		var jsmonth=this.state.dates.numberofmonth;
		var jsyear=this.state.currYear;

		var start=document.URL.length;
        var jsuser=document.URL.substring(45,start);

        $.post('deleteres.php', { jsuser,jsday,jsmonth,jsyear}, function(result) {
        	alert(resutl);
		});
	...
	render: function render() {
		...
		return React.createElement(
			'div',
			null,
			//React.createElement('input', { type: 'hidden',id: 'entry_nameU',value:'holi2'}),
			React.createElement(
				'div',
				{ id: 'calendar' },
				React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									'span',
									{ id: 'madeName' },
									'Juan Fernando y Maria Paula'
								),
								React.createElement(
									'span',
									{ id: 'madeInfo' },
									'WebDeveloper - Ingesoft'
								),
								React.createElement(
									'span',
									{ id: 'madeWeb' },
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement('input', { type: 'text', placeholder: 'Nombre reserva', id: 'entry_name' }),
						React.createElement(
							'span',
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									...
									React.createElement(
										'p',
										{ id: 'hour1' },
										React.createElement('input', { type: 'number', id: 'enter_hour1', min: '0', max: '24' }),
										' h'
									),
									React.createElement(
										'p',
										{ id: 'hour2' },
										React.createElement('input', { type: 'number', id: 'enter_hour2', min: '0', max: '24' }),
										' h'
									)
								)
							),
							React.createElement(
								...
								React.createElement('input', { type: 'text', placeholder: 'Zona a reservar', id: 'entry_location' })
							),
							React.createElement(
								'div',
								{ className: 'entry_info2' },
								React.createElement('i', { className: 'fa fa-pencil', 'aria-hidden': 'true' }),
								React.createElement('textarea', { id: 'entry_note', cols: '35', rows: '2', placeholder: 'Agregar nota' })
							),
							React.createElement(
								'div',
								...
								if (!done) {
									...
									return React.createElement(
										...
										React.createElement(
											'span',
											null,
											'No hay eventos planeados para hoy'
										)
									);
								}
							}
...
var x=ReactDOM.render(React.createElement(Calendar, null), document.getElementById("app"));
